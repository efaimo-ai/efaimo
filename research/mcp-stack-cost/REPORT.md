...
42,959 tokens is about **4.3% of a 1M context window**, or **21.5% of a 200k
one**, loaded on every request before the agent reads a single thing.

> Corrected 2026-07-18: this total first went out as 58,959, which was an
> addition error in a hand-summed column. The per-server rows were always
> right. It is now generated by `scripts/stack-cost.mjs`, which runs the CLI
> per server and sums the result, so the total cannot disagree with the table
> again. A tool that exists to make numbers defensible does not get to add up
> its own by hand.

## What actually drives the number

The distribution is what matters, not the total. Two servers are 79% of the
